...
export default function PrediksiPanen() {
    ...
    const fetchPredictionData = async () => {
        ...
        setError(null);
        try {
            const response = await fetch('/api/predictions/data');
            if (!response.ok) throw new Error('Failed to fetch prediction data');

            const data = await response.json();
            setPredictionData({
                estimatedDate: data.overall.estimated_harvest_date,
                daysLeft: data.overall.days_left,
                totalFruit: data.overall.total_fruits,
                qualityScore: data.overall.avg_quality_score,
                expectedYield: data.overall.expected_yield_ton + ' ton',
            });

            setBlockPredictions(data.blocks.map(b => ({
                block: b.block_code,
                readiness: b.readiness,
                fruits: b.fruits,
                harvestDate: b.harvest_date_range,
                quality: b.quality,
            })));

            setWeekTrend(data.weekly_trend);
        } catch (err) {
            console.error('Error fetching predictions:', err);
            setError('Gagal memuat data prediksi. Pastikan ada data deteksi kematangan.');
        } finally {
            setIsLoading(false);
        }
    };
    ...
    const handleExportCSV = async () => {
        setIsExporting(true);
        try {
            const response = await fetch('/api/predictions/export/csv');
                ...
                <div className="relative p-4 md:p-6 space-y-4 md:space-y-6 max-w-7xl mx-auto">
                        ...
                        <Card className="p-5 md:p-8 bg-gradient-to-br from-green-50 via-yellow-50 to-orange-50 border-2 border-green-300 shadow-xl">
                            <div className="text-center mb-6">
                                ...
                                <p className="text-3xl font-bold text-green-600 mb-2">
                                    {predictionData?.estimatedDate || 'Memuat...'}
                                </p>
                    ...
                    >
                        <Button
                            ...
                            className="w-full bg-gradient-to-r from-green-600 to-emerald-600 hover:from-green-700 hover:to-emerald-700 flex items-center justify-center gap-2 shadow-lg h-12 disabled:opacity-50"
                        >
                            <FileText className="w-5 h-5" />
                            {isExporting ? 'Sedang Export...' : '📊 Export CSV'}
                        </Button>
                    </motion.div>
